...
for doc_index, document in enumerate(documents):
    ...
    # Vectorizing singleton words using FastText
    singleton_docs = [list(singleton_words)]

    # Create FastText model
    fasttext_model = FastText(singleton_docs, vector_size=100, window=2, min_count=1, sg=1)

    # Create vectors for singleton words
    word_vectors = np.array([fasttext_model.wv[word] for word in singleton_words])

    # PCA on FastText
    pca_ft = PCA(n_components=min(word_vectors.shape[0], word_vectors.shape[1]))
    X_ft_pca = pca_ft.fit_transform(word_vectors)

    # Get the explained variance ratio
    explained_variance_ratio_ft = pca_ft.explained_variance_ratio_

    # Find the component with the least explained variance
    least_variance_component_ft = np.argmin(explained_variance_ratio_ft)

    # Find the corresponding word with the least variance
    least_occuring_word_ft = list(singleton_words)[least_variance_component_ft]

    print(f"Least occurring word in FastText PCA: {least_occuring_word_ft}")

    # Collect all words for the document
    all_words = list(high_co_occurrence_words) + [least_occuring_word_ft]
...
# Vectorize all documents using FastText
all_docs = [doc.split() for doc in processed_docs]

# Create FastText model
fasttext_model = FastText(all_docs, vector_size=100, window=2, min_count=1, sg=1)

# Create vectors for all documents
doc_vectors = np.array([np.mean([fasttext_model.wv[word] for word in doc], axis=0) for doc in all_docs])
...
for i, file_path in enumerate(file_paths):
    ...
    plt.annotate(document_name, (doc_vectors_pca[i, 0], doc_vectors_pca[i, 1]))
'''
plt.show()
