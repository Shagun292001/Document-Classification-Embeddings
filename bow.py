...
for doc_index, document in enumerate(documents):
    ...
    # Vectorizing singleton words using Bag of Words
    singleton_docs = [" ".join([word]) for word in singleton_words]

    # Create Bag of Words vectorizer
    bow_vectorizer = CountVectorizer()
    X_bow = bow_vectorizer.fit_transform(singleton_docs).toarray()

    # Get the feature names (singleton words)
    bow_feature_names = bow_vectorizer.get_feature_names_out()

    # PCA on Bag of Words
    pca_bow = PCA(n_components=min(X_bow.shape[0], X_bow.shape[1]))
    X_bow_pca = pca_bow.fit_transform(X_bow)

    # Get the explained variance ratio
    explained_variance_ratio_bow = pca_bow.explained_variance_ratio_

    # Find the component with the least explained variance
    least_variance_component_bow = np.argmin(explained_variance_ratio_bow)

    # Find the corresponding word with the least variance
    least_occuring_word_bow = bow_feature_names[least_variance_component_bow]

    print(f"Least occurring word in Bag of Words PCA: {least_occuring_word_bow}")

    # Collect all words for the document
    all_words = list(high_co_occurrence_words) + [least_occuring_word_bow]
...
# Vectorize all documents using the same Bag of Words vectorizer
bow_vectorizer = CountVectorizer()
X_all_bow = bow_vectorizer.fit_transform(processed_docs).toarray()
...
kmeans.fit(X_all_bow)
labels = kmeans.labels_

# Perform PCA to reduce dimensions for plotting
pca = PCA(n_components=2)
X_all_bow_pca = pca.fit_transform(X_all_bow)

# Plotting KMeans clusters on the same graph
plt.figure(figsize=(12, 6))
plt.scatter(X_all_bow_pca[:, 0], X_all_bow_pca[:, 1], c=labels, cmap='viridis')
...
silhouette_avg = silhouette_score(X_all_bow, labels)
davies_bouldin = davies_bouldin_score(X_all_bow, labels)
